...
rather than a porcelain command (human-readable). That makes it the right tool
for shell scripts, but its safety depends entirely on how inputs and outputs are
handled.
...
Resolve standard Git paths regardless of the caller's `cwd`. Hardcoding paths in
hooks or CI is brittle.
...
`--verify` requires _exactly one_ parameter and ensures it resolves to a valid
Git object. If the user passes `--all`, `--verify` errors out instead of
silently dumping every ref.
...
A branch and a file may share a name. Use `--` to declare "everything after this
is a path."
...
If `USER_INPUT` is `--all`, the unsafe form returns _every ref_, turning `$HASH`
into a multi-line monster that breaks the next consumer (e.g.
`git checkout $HASH`). `--verify` constrains the output to a single ref or a
non-zero exit.
...
Outside a Git repo, `git rev-parse` fails and `$ROOT` is empty. Combine with an
unguarded `rm -rf` and you delete the wrong directory tree.
...
If `USER_INPUT=$'HEAD; rm -rf /'`, the unquoted form lets the shell re-interpret
the `;` and execute the second command. Always quote.

### Pattern D — Misusing `--parseopt` + `eval`

`git rev-parse --parseopt` parses flags for shell scripts. Its output is
designed to be `eval`'d, which means a malformed spec string or unquoted output
is an arbitrary-code-execution vector inside the script.
...
Either treat the eval site like any other untrusted-input boundary (static spec
string, never user-controlled), or skip `--parseopt` and parse flags some other
way.
...
Patterns B and C are not auto-detected because they require multi-line context
the hook doesn't see; rely on this doc + review for those.

## If the hook warns

Read the message, decide whether the pattern is genuinely unsafe in the context,
and (if so) rewrite using the matching ✅ form above. The hook does **not**
block — proceed if you are sure.
