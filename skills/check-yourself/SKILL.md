...
| "I'm in subagent-driven mode (/cat); inter-task /check-yourself is overkill"                                                                                                                                           | The trigger enumeration says "a step in a multi-step plan is marked complete." The dispatch model does not change what counts as a step. If TaskUpdate flips status to `completed` — or, when the Task tools are absent from the tool surface, a row in the Markdown fallback table/list flips to done — that IS the boundary — fire now, don't batch the catch-up to the next /pause.                                                                                                               |
