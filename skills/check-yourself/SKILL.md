...
1. **Update the task list.** Display the current ` ◼` / ` ✔` task list (at the
   ...
   - Some sessions gate `TaskCreate` / `TaskUpdate` / `TaskList` / `TaskGet` off
     the tool surface entirely (a GrowthBook experiment flag; legacy `TodoWrite`
     can be absent too). When those tools are absent, "the current task list" is
     whatever Markdown ◼/✔ list or table the agent is already maintaining by
     hand — Step 1 still applies the same way, just against that Markdown state
     instead of a tool call. State once, the first time you notice the tools are
     gone, that you're tracking state in Markdown instead; don't repeat the
     caveat at every later boundary.
...
| "I'm in subagent-driven mode (/cat); inter-task /check-yourself is overkill"                                                                                                                                           | The trigger enumeration says "a step in a multi-step plan is marked complete." The dispatch model does not change what counts as a step. If TaskUpdate flips status to `completed` — or, when the Task tools are absent from the tool surface, a row in the Markdown fallback table/list flips to `completed` — that IS the boundary — fire now, don't batch the catch-up to the next /pause.                                                                                                        |
