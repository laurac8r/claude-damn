...
them to turn it on (press `a` or `/allowed-tools`) and wait for them to enable it
before dispatching — otherwise even backgrounded dispatch stops on every edit for
approval.
...
per-edit approval is **separate from** `/subagent-driven-development`'s two-stage
spec-then-quality review — approving a diff does not stand in for that review; the
...
changes WHO APPROVES each edit (you, interactively) — not WHO MAKES it (still the
dispatched subagent). "I'm approving every diff in the foreground anyway, so I
may as well edit directly" is the same shortcut as every row below, wearing a new
hat. The execution model the user chose holds in every approval mode.
