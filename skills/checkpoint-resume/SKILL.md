...
- **Active checkpoint:** Check whether `CHECKPOINT.md` exists at CWD. If it does, parse its `**Branch:**` line.
- **Archive:** List `$ARCHIVE/*.md`, excluding any file matching `*.prev.md` (those are rolling same-branch backups, not
  paused efforts — do not offer them for resume).
...
Once a `CHECKPOINT.md` is confirmed at CWD (either pre-existing or moved there in Step 4):
...
3. Read any memory files referenced in the checkpoint's **Relevant Memories** section.
4. Review the **Branch**, **Current State**, **Key Decisions Made**, and **Open Questions / Blockers** sections — flag
   any drift between the checkpoint's "Files Modified" list and actual file state.
5. Summarize the restored context to the user: what was done, what decisions were made, what is next.
...
- The archive no longer contains the restored file — it was **moved**, not copied.
- The archive may still contain other paused efforts for other branches.

## Key Principles

- **Trust the checkpoint:** It was written with full session context. Don't second-guess decisions unless the user asks
  to revisit them.
- **Flag drift:** If files changed since the checkpoint was written (someone else committed, user edited manually), call
