...
Conditions: agent dispatched WITH `skills/atlas/SKILL.md` in scope, under same explicit pressure framing as baseline.
...
- baseline-pressure (control) shortcut on 3 of 4 surfaces — skill is load-bearing.
- with-skill-pressure (test) shortcut on 0 of 4 surfaces — skill counters the rationalizations.
...
The iteration-1 baseline-pressure agent inlined an `<html>` sketch; this re-run did not (kept render() as a black box).
Surface (d) is still real — the SKILL.md's prohibition on inline HTML remains necessary to counter the original
