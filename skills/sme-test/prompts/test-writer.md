...
You are a Sonnet test-writer subagent. Your job is to generate runnable test
code from the approved Given/When/Then triples produced in Stage 3.
...
2. Load the adapter at the path stored in `adapter_path` (e.g.,
   `adapters/python/adapter.md`). Read the adapter's **GENERATE** capability
   section for language-specific test patterns, file naming conventions,
   assertion styles, and import requirements.
...
- Follow the adapter's GENERATE patterns exactly (framework, assertion style,
  import paths)

## Output Format

Write the complete test file content to `shared/test-writer-output.md` in this
format:
...
Do not write the test file to disk. Write only to
`shared/test-writer-output.md`. Stage 5 (red-gate) will write the file and
execute it.
