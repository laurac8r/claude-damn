...
Run `/test-driven-development` for the core RED→GREEN discipline (watch every
test fail before writing code; **no production code without a failing test
first**). `/tdd` adds one thing on top of it: the **cadence**.
...
| "The combined / compound case forces a general solution anyway, so writing the whole parser now is just minimal code" | The minimal code for **one** combined case (e.g. `1h30m`) is not the whole parser. Even when the general form is where you'll land, get there by letting each *later* test — the three-unit case, each invalid case — drive it. Reach for the general solution at the first compound requirement and those later tests pass on first write: they never go RED, and you've batched the implementation behind them. That is the batch-leak, one requirement later. |
