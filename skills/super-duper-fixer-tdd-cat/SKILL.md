...
pre-implementation), skip /tdd (no test target yet), skip /duper (no
worktree needed when no edits are planned), and skip /cat (no implementation
to parallelize). Invoke /brainstorming directly, produce the design
artifact (visual-aid, prose, or spec-stub per operator preference), and STOP
at the design gate. Do not dispatch to TDD/implementation subagents. The
brainstorm artifact IS the deliverable; follow-up session picks up via
/checkpoint-resume.
