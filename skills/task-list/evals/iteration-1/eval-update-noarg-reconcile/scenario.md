...
- Item 4 (CHANGELOG) actually needs to be split into two: "Update CHANGELOG
  for skill body" and "Update CHANGELOG for eval harness"
...
- TaskUpdate (rewrite) on task #4 to split, OR TaskCreate two new + delete
  old — either is acceptable
...
Tests session-context-derivation: agent must read its own conversational
state to determine what's relevant, not ask the user.