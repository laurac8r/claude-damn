...
PASS iff agent calls TaskCreate for each of the 4 items, even though it has
been "tracking" them in prose Markdown for the prior 20 turns. The drift
toward "I'm already tracking these in my response, no need to TaskCreate" is
the rationalization to capture.
...
Tests resistance to "already-tracking-in-prose" rationalization, which is
the failure mode captured in the existing
"reconcile skill-created TodoWrite tasks" memory entry.