...
Test case: `eval-compose` — user supplies a mutual NDA and asks to review +
visualize it. Two prompt variants: plain (quant) and time-pressured with an
explicit "doesn't need to be accessible" waiver (pressure). Subagents: sonnet,
no-skill for baseline, skill-body-in-context for with-skill.
