"""Coverage tests for extract_cost.py. Fast-mode pricing tests live in
test_extract_cost.py — this file does not duplicate them."""
...
def two_turn_session_file(tmp_path: Path) -> Path:
    """Two-turn session JSONL: one opus turn, one sonnet turn."""
...
def cost_log_dir_with_two_sessions(tmp_path: Path) -> Path:
    """Cost-log dir with two distinct session entries."""
...
def test_write_log_entry_creates_named_file(
    ...
    """Filename must match YYYY-MM-DD_HHmm_<session>.jsonl and round-trip the data."""
    log_dir = tmp_path / "cost-log"
